...
| `agent_registration_endpoint` | string | Yes | URL for agent registration |
| `supported_providers` | array | Yes | List of available providers |
| `supported_providers[].provider_id` | string | Yes | Unique provider identifier |
| `supported_providers[].display_name` | string | Yes | Human-readable name |
| `supported_providers[].available_scopes` | string[] | Yes | Scopes that can be requested |
...
Services publish ATH metadata at a well-known endpoint for direct agent-to-service discovery.
