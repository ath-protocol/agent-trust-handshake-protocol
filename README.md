...
---
## 🔄 协议工作流程
ATH的工作流程就像现实中的商务会面，非常好理解：
### 第一步：身份报备 🪪
AI代理提前在ATH系统注册，获得唯一的数字身份证书，就像办了一张"AI身份证"。
### 第二步：握手申请 🤝
AI想要访问某个服务时，向ATH网关发起握手申请，说明"我是谁，我要做什么，需要什么权限"。
### 第三步：双向验证 🔍
- ATH系统验证AI的身份是否合法
- 同时验证要访问的服务是否是合法的ATH认证服务
### 第四步：用户授权 ✅
系统把访问请求推送给用户，用户确认"同意这个AI访问这个服务的这些权限"。
### 第五步：服务确认 🆗
服务端也确认"同意为这个AI提供服务，权限范围是XXX"。
### 第六步：发放令牌 🎫
双方都同意后，ATH系统发放临时访问令牌，AI就可以用这个令牌访问服务了。
### 第七步：访问记录 📝
整个过程和后续的访问操作都会被记录下来，永久存证。
### 第八步：令牌失效 ⏰
任务完成或者令牌到期后，令牌自动失效，AI需要重新申请握手。
# 握手流程
ATH协议的核心是12步可信握手流程，在客户端和服务端之间建立双向可信的通信通道。
## 12步握手流程概览
```mermaid
sequenceDiagram
    participant C as 客户端
    participant S as 服务端
    participant R as 身份注册中心
    
    Note over C,S: 第一阶段：身份验证（6步）
    C->>S: 步骤1: 客户端握手请求（DID + 能力清单）
    S->>R: 步骤2: 验证客户端身份有效性
    R->>S: 步骤3: 返回客户端身份验证结果
    S->>C: 步骤4: 服务端握手响应（自身DID + 能力清单）
    C->>R: 步骤5: 验证服务端身份有效性
    R->>C: 步骤6: 返回服务端身份验证结果
    
    Note over C,S: 第二阶段：权限协商（3步）
    C->>S: 步骤7: 权限请求清单（接口+数据范围）
    S->>C: 步骤8: 权限审批结果（同意/拒绝/部分同意+有效期）
    C->>S: 步骤9: 权限范围确认
    
    Note over C,S: 第三阶段：会话建立（3步）
    C->>S: 步骤10: 会话密钥协商请求
    S->>C: 步骤11: 会话密钥协商响应
    S->>C: 步骤12: 双方建立加密通信会话
```
...
#### 步骤2：服务端发起身份验证 (Server Initiate Identity Verification)
服务端接收到客户端请求后，向身份注册中心验证客户端身份的有效性：
- 发送客户端DID到身份注册中心
- 请求验证客户端身份是否合法、是否被吊销
#### 步骤3：注册中心返回验证结果 (Registry Return Verification Result)
身份注册中心返回客户端身份验证结果：
- 身份是否有效
- 身份属性信息
- 信用等级等附加信息
#### 步骤4：服务端身份回应 (Server Identity Response)
服务端回应客户端的身份宣告，返回自身身份信息：
- **服务端DID**：服务端的去中心化身份标识
- **协商后的协议版本**：从客户端提供的版本列表中选择的双方都支持的最高版本
- **服务端能力集**：服务端支持的加密算法、认证方式等能力
- **服务端元数据**：包含服务端点信息、支持的作用域列表、令牌有效期等配置
#### 步骤5：客户端发起身份验证 (Client Initiate Identity Verification)
客户端向身份注册中心验证服务端身份的有效性：
- 发送服务端DID到身份注册中心
- 请求验证服务端身份是否合法、是否为官方认证服务
#### 步骤6：注册中心返回验证结果 (Registry Return Verification Result)
身份注册中心返回服务端身份验证结果：
- 身份是否有效
- 服务资质信息
- 安全等级等附加信息
### 第二阶段：权限协商阶段（3步）
#### 步骤7：作用域请求 (Scope Request)
客户端向服务端请求需要访问的资源作用域：
- **权限列表**：客户端需要访问的资源和操作权限，格式为`资源:操作`（如`user:read`, `data:write`）
- **访问有效期**：客户端请求的访问凭证有效期
- **请求上下文**：可选的上下文信息，用于说明访问目的、业务场景等
#### 步骤8：作用域协商 (Scope Negotiation)
服务端根据自身安全策略对客户端请求的作用域进行审核和协商：
- **同意的作用域列表**：服务端最终同意授予客户端的权限范围
- **拒绝的作用域及原因**：对于拒绝的权限，返回明确的拒绝原因
- **访问限制条件**：对授予的权限附加的限制条件，如IP限制、速率限制等
#### 步骤9：权限确认 (Permission Confirmation)
客户端确认最终的权限范围，与服务端达成一致：
- 确认同意服务端授予的权限列表
- 确认访问限制条件
- 确认令牌有效期等配置
### 第三阶段：会话建立阶段（3步）
#### 步骤10：密钥协商请求 (Key Negotiation Request)
客户端发起会话密钥协商请求：
- 支持的密钥交换算法列表
- 支持的加密套件列表
- 密钥协商参数
#### 步骤11：密钥协商响应 (Key Negotiation Response)
服务端返回密钥协商响应：
- 选择的密钥交换算法
- 选择的加密套件
- 服务端密钥协商参数
#### 步骤12：握手完成 (Handshake Complete)
双方完成密钥协商，建立加密通信会话：
- 生成共享会话密钥
- 验证密钥有效性
- 正式建立端到端加密通信通道
- 后续所有业务请求都使用该会话密钥加密传输
## 安全特性
- **去中心化身份**：采用DID身份体系，不依赖单一权威机构
- **双向身份认证**：双方都通过独立注册中心验证对方身份，防止中间人攻击
- **最小权限原则**：服务端只授予客户端必要的访问权限
- **短期凭证**：访问凭证有效期短，降低凭证泄露风险
- **端到端加密**：TLS 1.3加密传输，支持前向保密
...
│   ├── 0.1/                        # v0.1版本协议
│   │   ├── client/                  # 客户端协议规范（原basic目录）
│   │   │   ├── handshake-flow.mdx  # 【核心】可信握手12步流程详细定义
│   │   │   ├── identity.mdx        # 身份认证标准，AI数字身份格式定义
│   │   │   ├── security.mdx        # 安全加密标准，防攻击规范
│   │   │   └── scope-intersection.mdx # 权限控制标准，最小权限实现
│   │   └── server/                 # 服务端协议规范
│   │       ├── authorization.mdx   # 【核心】授权决策逻辑规范
│   │       ├── token.mdx           # 访问令牌格式、生成、验证标准
│   │       ├── registration.mdx    # 代理和服务注册流程规范
│   │       └── revocation.mdx      # 令牌吊销流程规范
...
│
├── 📝 spec/                        # 接口标准定义
│   └── openapi.yaml                # OpenAPI 3.1格式，所有HTTP接口的权威定义，SDK和服务端实现都要遵循
│
