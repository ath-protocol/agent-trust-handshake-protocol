...
---
# 握手流程
ATH协议的核心是三方参与的9步可信握手流程，包含**智能体(客户端)**、**应用(服务端)**、**用户**三个独立角色，实现"用户+服务"双重授权机制，无需任何中心化身份注册中心。
## 9步握手流程概览
```mermaid
sequenceDiagram
    participant U as 用户
    ...
    Note over U,C,S: 第二阶段：双重授权协商（3步）
    C->>S: 步骤5: 权限请求（请求的作用域 + 用户授权凭证）
    S->>U: 步骤6: 服务端向用户确认授权（可选，按需触发）
    U->>S: 步骤7: 用户返回授权确认结果
...
> ✅ **双重授权机制**：智能体要成功访问服务，必须同时获得两个授权，缺一不可：
> 1. **用户授权**：用户同意该智能体代表其访问指定资源
> 2. **服务授权**：服务端同意该智能体访问自身提供的服务
## 详细步骤说明
### 前置步骤：用户预授权
#### 步骤0：用户向智能体授予委托权限
...
#### 步骤6：服务端向用户确认授权（可选）
根据服务的安全策略，服务端可以选择直接信任用户预授权凭证，或者向用户二次确认：
- 服务端向用户发送授权确认请求，包含智能体身份、请求的权限范围
- 这一步是可选的，安全等级高的服务可以启用，普通服务可以跳过
#### 步骤7：用户返回授权确认结果
用户确认授权请求，返回确认结果：
- 用户可以选择同意、拒绝或修改授权范围
- 确认结果由用户签名，具有法律效力
